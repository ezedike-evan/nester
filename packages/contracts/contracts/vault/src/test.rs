...
fn test_pause_and_unpause_are_idempotent() {
    let (_env, admin, _token_address, _contract_id, client) = setup();
...
#[test]
fn test_cross_contract_pause_state_is_visible() {
    let (env, admin, _token_address, contract_id, client) = setup();
    ...
    assert!(!observer.is_target_paused(&contract_id));
...
#[test]
fn test_cross_contract_admin_can_pause_target() {
    let (env, admin, _token_address, contract_id, client) = setup();
    let observer_id = env.register_contract(None, VaultObserverContract);
    let observer = VaultObserverContractClient::new(&env, &observer_id);

    observer.pause_target(&contract_id, &admin);
    assert!(client.is_paused());
}

#[test]
#[should_panic]
fn test_cross_contract_non_admin_cannot_pause_target() {
    let (env, admin, _token_address, contract_id, _client) = setup();
    let observer_id = env.register_contract(None, VaultObserverContract);
    let observer = VaultObserverContractClient::new(&env, &observer_id);
    let outsider = Address::generate(&env);

    let _ = admin;
    observer.pause_target(&contract_id, &outsider);
}
